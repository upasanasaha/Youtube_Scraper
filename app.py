...
def channel_info():
    ...
    '''
    try:
        channel_info = get_channel_stats(Youtube_Scraper, obj_ytube.channel_id)

        chan_col.insert_one(channel_info)
        video_ids = get_video_id(Youtube_Scraper, channel_info)
        video_info = get_video_details(Youtube_Scraper, video_ids)
        for i in video_info:
...
def comment(path):
    ...
    try:
        ...
        for i in a:
            ...
            vid_info = get_vid_detail(Youtube_Scraper, path)
            comment_table = get_comment_table(Youtube_Scraper, vid_info)
